...
function LinkGenerated({data,onPreview}){
      ...
      <GlassCard glow>
        <div style={{textAlign:"center"}}>
          ...
          <p style={{color:C.textSoft,fontSize:"14px",marginBottom:"20px"}}>
            Share this with <strong style={{color:C.rose}}>{data.friendName}</strong> 💖<br/>
            <span style={{fontSize:"12px"}}>They'll see the full surprise — creator form hidden!</span>
          </p>

          {/* Link preview box */}
          <div style={{background:"rgba(255,182,193,.15)",border:"1.5px dashed #FFB6C1",borderRadius:"16px",padding:"12px 16px",marginBottom:"16px",fontFamily:"'Poppins',sans-serif",fontSize:"11px",color:C.textSoft,wordBreak:"break-all",textAlign:"left",maxHeight:"72px",overflow:"hidden"}}>
            {url.slice(0,90)}...
          </div>

          <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
            {/* 👁 Preview button */}
            <motion.button whileHover={{scale:1.03}} whileTap={{scale:.97}} onClick={onPreview} style={{
              background:"linear-gradient(135deg,#a78bfa,#7c3aed)",color:"#fff",border:"none",borderRadius:"50px",padding:"14px",
              fontFamily:"'Fredoka',cursive",fontSize:"16px",fontWeight:600,cursor:"pointer",
              display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",
              boxShadow:"0 4px 16px rgba(124,58,237,.35)",
            }}>👁️ Preview Surprise</motion.button>

            <motion.button whileHover={{scale:1.03}} whileTap={{scale:.97}} onClick={copy} style={{
              background:copied?"#25a244":"linear-gradient(135deg,#FF6B8A,#FF4070)",color:"#fff",border:"none",borderRadius:"50px",padding:"14px",
              fontFamily:"'Fredoka',cursive",fontSize:"16px",fontWeight:600,cursor:"pointer",
              display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",
...
export default function App(){
  // mode: null=loading | "creator" | "receiver" | "preview"
  ...
  // On mount: check URL for ?s= param → receiver mode
  useEffect(()=>{
    ...
    if(encoded){
      try{
        ...
        setMode("receiver");
        setScreen("intro"); // always start receiver from cinematic intro
      }catch{
        setMode("creator");
      }
    }else{
  ...
  // Preview: creator previews the surprise using their own data
  const startPreview=useCallback(()=>{
    setReceiverData(creatorData);
    setScreen("intro");
    setMode("preview");
  },[creatorData]);
  ...
  // Active data — works for both receiver and preview
  const d=receiverData;
  const name=d?.friendName||"Bestie";

  // Build receiver/preview screen map
  const buildScreens=(d,name)=>({
    ...
    photos:     <PhotoStory photos={d?.photos||[]} friendName={name} onDone={()=>go("movie")}/>,
    movie:      <MemoryMovie photos={d?.photos||[]} friendName={name} onNext={()=>go("polaroid")}/>,
    polaroid:   <PolaroidWall photos={d?.photos||[]} onNext={()=>go("flowers")}/>,
    flowers:    <FlowersScreen friendName={name} onNext={()=>{
                  if(d?.memories?.length>0)go("timeline");else go("wrapped");
                }}/>,
    timeline:   <MemoryTimeline memories={d?.memories||[]} onNext={()=>go("wrapped")}/>,
    wrapped:    <FriendshipWrapped data={d||{}} onNext={()=>go("whyspecial")}/>,
    whyspecial: <WhySpecial friendName={name} onNext={()=>go("promises")}/>,
    promises:   <FuturePromises promise={d?.futurePromise} friendName={name} onNext={()=>{
    ...
    meter:      <FriendshipMeter friendName={name} onDone={()=>go("capsule")}/>,
    capsule:    <MemoryCapsule data={d||{}} onNext={()=>go("celebrate")}/>,
    celebrate:  <CelebrationScreen data={d||{friendName:"Bestie"}}/>,
  });

  const activeScreens=useMemo(()=>buildScreens(d,name),[d,name,screen]);
  ...
  return(
    <>
      ...
      {/* Dark mode toggle — only in receiver/preview, not on intro */}
      {(mode==="receiver"||mode==="preview")&&screen!=="intro"&&(
      ...
      {mode==="preview"&&(
        <motion.button whileHover={{scale:1.05}} whileTap={{scale:.95}} onClick={exitPreview}
          style={{position:"fixed",top:"16px",right:"16px",zIndex:400,background:"rgba(124,58,237,.85)",border:"none",
            borderRadius:"50px",padding:"8px 16px",color:"#fff",fontFamily:"'Fredoka',cursive",fontSize:"14px",
            cursor:"pointer",backdropFilter:"blur(8px)",boxShadow:"0 4px 16px rgba(124,58,237,.4)"}}>
          ← Exit Preview
        </motion.button>
      )}
        ...
        <AnimatePresence mode="wait">
          {!creatorDone?(
            ...
            <motion.div key="link" initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-24}} transition={{duration:.4}}>
              <LinkGenerated data={creatorData} onPreview={startPreview}/>
            </motion.div>
          )}
        </AnimatePresence>
      )}

      {/* ── RECEIVER / PREVIEW mode ── */}
      {(mode==="receiver"||mode==="preview")&&(
        <AnimatePresence mode="wait">
